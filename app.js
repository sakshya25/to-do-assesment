function shuffle(array) {
  ...
  //         <input v-else v-model="editedText" @keyup.enter="saveEdit" @blur="saveEdit" tabindex="0"/>
  //       </div>
  ...
  //       this.$emit('update-todo', { id: this.todo.id, text: this.editedText });
  //     },
  //   },
  // });
  Vue.component("todo-item", {
    ...
    template: `
      <li class="todo-item">
        <div class="todo-item-left">
          <span class="material-icons reorder">reorder</span>
          <!-- Display the todo item text -->
          <p v-if="!isEditing" @click="startEditing" :class="{completed: todo.completed}" tabindex="0">{{ todo.text }}</p>
          <!-- Display an input field when editing -->
          <input v-else v-model="editedText" @keyup.enter="saveEdit" @blur="saveEdit" tabindex="0"/>
        </div>
